...
APL_DOCS = {
    ...
    'present': _load_apl_document('./apl/present.json')
}
...
def present_screen(handler_input):
    if(supports_apl(handler_input)):
        handler_input.response_builder.add_directive(
            RenderDocumentDirective(
                token="presentToken",
                document=APL_DOCS['present'],
                datasources=generateLaunchScreenDatasource(handler_input)
            )
        )
...
def generateLaunchScreenDatasource(handler_input):
    ...
    saucesIdsToDisplay = ["TREE", "SAP", "POD",
                          "OPOD", "GPOD", "BLEAF", "MES", "MES-POD"]
