...
class PresentIntentHandler(AbstractRequestHandler):
    def can_handle(self, handler_input):
        return is_intent_name("present")(handler_input)
    ...
    def handle(self, handler_input):
        ...
        reprompt_output = data[prompts.WELCOME_REPROMPT]
        # Add APL Template if device is compatible
        apl_utils.present_screen(handler_input)
        # Generate JSON Response
        return handler_input.response_builder.speak(speak_output).ask(reprompt_output).response
...
class ResponseActionnableHistoryInterceptor(AbstractResponseInterceptor):
    ...
    def process(self, handler_input, response):
        ...
        if(current_request.object_type == 'IntentRequest'):
            ...
            if(record_request['intent']['name'] == "RecipeIntent" or record_request['intent']['name'] == "AMAZON.HelpIntent" or record_request['intent']['name'] == "present"):
...
sb.add_request_handler(PresentIntentHandler())
